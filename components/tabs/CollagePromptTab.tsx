...
import { generateCollagePrompt } from '../../services/geminiService';
import { InfoButton } from '../shared/InfoButton';
import { CopyButton } from '../shared/CopyButton';

export const CollagePromptTab = React.memo(({ showModal, showLoadingModal, hideLoadingModal, formData, setFormData, onReset }: { showModal: any, showLoadingModal: any, hideLoadingModal: any, formData: any, setFormData: any, onReset: any }) => {
    const [isDragging, setIsDragging] = useState(false);

    const { uploadedFile, collageCount, generatedPrompt } = formData;
    ...
    const handleFile = useCallback((file: File) => {
        ...
        setFormData((prev: any) => ({...prev, uploadedFile: { file, previewUrl: URL.createObjectURL(file) }, generatedPrompt: '' }));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        showLoadingModal('Designing Collage', `The AI is designing a ${collageCount}-part lifestyle sequence...`);
        setFormData((prev: any) => ({...prev, generatedPrompt: ''}));
        try {
            const base64ImageData = await fileToBase64(uploadedFile.file);
            const prompt = await generateCollagePrompt(base64ImageData, uploadedFile.file.type, collageCount);
    ...
    return React.createElement('div', null,
        React.createElement('div', { className: "flex justify-between items-center text-center mb-4" },
            ...
            React.createElement('h1', { className: "text-2xl md:text-3xl font-bold text-center flex-1" }, "Lifestyle Collage / Montage"),
            React.createElement('div', { className: "flex-1 flex justify-end" },
                React.createElement(InfoButton, { title: "About Collage Prompts" },
                    React.createElement('p', null, "This tool generates a single, detailed prompt that describes a sequence of lifestyle moments or cuts featuring your product."),
                    React.createElement('p', { className: "mt-2" }, "Perfect for creating split-screen images (Midjourney) or fast-paced montage videos (Veo).")
                )
            )
        ),
        React.createElement('p', { className: "text-center text-slate-400 mb-8" }, "Generate a multi-scene lifestyle prompt from a single product image."),
            ...
            React.createElement('div', { className: "grid grid-cols-1 md:grid-cols-2 gap-6 items-start" },
                React.createElement('div', {
                    ...
                    uploadedFile ? React.createElement(React.Fragment, null,
                        ...
                        React.createElement('label', { htmlFor: "collage-image-upload", className: "cursor-pointer text-sm text-purple-400 hover:text-purple-300 font-medium" }, "Click to upload"),
                        React.createElement('p', { className: "text-xs text-slate-400 mt-1" }, "or drag & drop product image")
                    ),
                    React.createElement('input', { type: "file", id: "collage-image-upload", className: "hidden", accept: "image/jpeg,image/png", onChange: (e: any) => handleFile(e.target.files[0]) } as any)
                ),
                React.createElement('div', { className: "space-y-4" },
                    React.createElement('div', null,
                        React.createElement('label', { htmlFor: "collageCount", className: "block text-sm font-medium mb-1 text-slate-300" }, "Collage Sections / Cuts"),
                        React.createElement('select', {
                            id: "collageCount",
                            name: "collageCount",
                            value: collageCount,
                            onChange: (e: any) => setFormData((prev: any) => ({...prev, collageCount: parseInt(e.target.value) })),
                            ...
                            React.createElement('option', { value: 2 }, "2 Sections / Cuts"),
                            React.createElement('option', { value: 3 }, "3 Sections / Cuts"),
                            React.createElement('option', { value: 4 }, "4 Sections / Cuts")
                        )
                    ),
                    React.createElement('div', { className: "text-xs text-slate-400 p-3 bg-slate-700 rounded-lg border border-slate-600" },
                        React.createElement('strong', { className: "text-purple-300" }, "Output Style: "),
                        "Quick, aesthetic POV cuts showing the product in use (e.g., holding coffee, driving, reading)."
                    )
                )
            ),
             React.createElement('div', { className: "flex flex-col sm:flex-row gap-4" },
                React.createElement('button', { type: "submit", className: "flex-1 w-full bg-purple-600 hover:bg-purple-700 disabled:bg-purple-800 disabled:cursor-not-allowed text-white font-bold py-3 px-4 rounded-lg transition duration-300 ease-in-out flex items-center justify-center space-x-2" },
                    ...
                    React.createElement('span', null, 'Generate Collage Prompt')
                ),
        ...
        generatedPrompt && React.createElement('div', { className: "mt-8" },
            React.createElement('h2', { className: "text-lg font-semibold mb-3 text-slate-200" }, "Generated Collage Prompt"),
            React.createElement('div', { className: "relative bg-slate-900 rounded-lg p-4 border border-slate-700" },
                React.createElement('textarea', { value: generatedPrompt, className: "w-full bg-transparent border-0 text-slate-300 focus:ring-0 resize-none pr-10 italic", rows: 5, readOnly: true }),
