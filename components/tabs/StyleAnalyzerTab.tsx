...
import { generateStyleAnalysisPrompt } from '../../services/geminiService';
import { InfoButton } from '../shared/InfoButton';
import { CopyButton } from '../shared/CopyButton';

export const StyleAnalyzerTab = React.memo(({ showModal, showLoadingModal, hideLoadingModal, formData, setFormData, onReset }: { showModal: any, showLoadingModal: any, hideLoadingModal: any, formData: any, setFormData: any, onReset: any }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        showLoadingModal('Analyzing Style', 'The AI art historian is deconstructing the image...');
        setFormData((prev: any) => ({...prev, generatedPrompt: ''}));
        try {
            const base64ImageData = await fileToBase64(uploadedFile.file);
            const prompt = await generateStyleAnalysisPrompt(base64ImageData, uploadedFile.file.type);
    ...
    return React.createElement('div', null,
        React.createElement('div', { className: "flex justify-between items-center text-center mb-4" },
            ...
            React.createElement('h1', { className: "text-2xl md:text-3xl font-bold text-center flex-1" }, "Visual Style Analyzer"),
            React.createElement('div', { className: "flex-1 flex justify-end" },
                React.createElement(InfoButton, { title: "About Style Analyzer" },
                    React.createElement('p', null, "This tool deconstructs the visual aesthetic of any image and turns it into a detailed prompt for an AI image generator."),
                    React.createElement('p', { className: "mt-2" }, "Upload an image, and the AI will analyze its style, lighting, color palette, and composition to create a prompt that can replicate that look.")
                )
            )
        ),
        React.createElement('p', { className: "text-center text-slate-400 mb-8" }, "Upload any image to reverse-engineer its aesthetic into a detailed prompt."),
        React.createElement('form', { onSubmit: handleSubmit, className: "space-y-6" },
            React.createElement('div', {
                ...
                uploadedFile ? React.createElement(React.Fragment, null,
                    ...
                    React.createElement('label', { htmlFor: "style-image-upload", className: "cursor-pointer text-sm text-purple-400 hover:text-purple-300 font-medium" }, "Click to upload"),
                    React.createElement('p', { className: "text-xs text-slate-400 mt-1" }, "or drag & drop an image to analyze")
                ),
                React.createElement('input', { type: "file", id: "style-image-upload", className: "hidden", accept: "image/jpeg,image/png", onChange: (e: any) => handleFile(e.target.files[0]) } as any)
            ),
             React.createElement('div', { className: "flex flex-col sm:flex-row gap-4" },
                React.createElement('button', { type: "submit", className: "flex-1 w-full bg-purple-600 hover:bg-purple-700 disabled:bg-purple-800 disabled:cursor-not-allowed text-white font-bold py-3 px-4 rounded-lg transition duration-300 ease-in-out flex items-center justify-center space-x-2" },
                    ...
                    React.createElement('span', null, 'Analyze Style')
                ),
        ...
        generatedPrompt && React.createElement('div', { className: "mt-8" },
            React.createElement('h2', { className: "text-lg font-semibold mb-3 text-slate-200" }, "Generated Style Prompt"),
